...
class TestPostProcessing(unittest.TestCase):
    ...
    def test_correct_transition_count(self):
        n_grid_points = 10
        n_transitions = 3
        in_prob, grid, cv_coordinates = create_constant_probability_distribution(n_grid_points=n_grid_points,
                                                                                 n_transitions=n_transitions)
        tc = TransitionCountCalculator(config=self.config,
                                       n_grid_points=n_grid_points,
                                       cv_coordinates=cv_coordinates)
        tc.run()
        self.assertEqual(grid.shape, tc.grid.shape)
        self.assertAlmostEqual(abs(grid - tc.grid).max(), 0, places=4, msg="Grids differ")
        print(tc.transition_count)
        self.assertTrue(np.all(tc.transition_count == n_transitions), "Transition count is constant")
    ...
    def test_correct_probability_distribution(self):
        ...
        tc = TransitionCountCalculator(config=self.config,
                                       ...
                                       cv_coordinates=cv_coordinates)
        tc.run()
        fc = FreeEnergyCalculator(config=self.config,
                                  grid=tc.grid,
        ...
        for row_idx, p_row in enumerate(in_prob):
            for col_idx, p_in in enumerate(p_row):
                ...
                self.assertAlmostEqual(p_out, p_in,
                                       places=4,
                                       msg="Probability not equal at index %s, %s" % (row_idx, col_idx))
        self.assertAlmostEqual(1., out_prob.sum(), places=4, msg="Total probability should equal 1")
    ...
    def test_two_state_1d_probability_distribution(self):
        ...
        tc = TransitionCountCalculator(config=self.config,
                                       ...
                                       cv_coordinates=cv_coordinates)
        tc.run()
        self.assertAlmostEqual(abs(in_transition_count - tc.transition_count).max(), 0, places=4,
