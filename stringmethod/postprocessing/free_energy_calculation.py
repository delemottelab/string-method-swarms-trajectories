...
class FreeEnergyCalculator(AbstractPostprocessor):
    ...
    def _do_persist(self):
        np.save("{}/probability_distribution".format(self._get_out_dir()), self.probability_distribution)
    ...
    def _compute_probability_distribution_eigenvector(self):
        ...
        transition_count = self._remove_transitions_to_isolated_bins(self.transition_count)
        ...
        for idx, eigenval in enumerate(eigenvalues):
            ...
            if np.isclose(1., eigenval, rtol=1e-2):
                ...
                if len(pos_vec) == 0:
                    ...
                    logger.warning("Found a vector with eigenvalue ~1(%s) but with negative entries in its eigenvector",
                                   eigenval)
                    continue
        ...
        if relaxation_eigenval is not None:
            logger.info("Relaxation time for system: %s [units of lag time]. Eigenval=%s", -np.log(relaxation_eigenval),
                        relaxation_eigenval)
    ...
    def _compute_probability_distribution_detailed_balance(self):
        ...
        if inaccessible_states > 0 or nonstarting_states > 0:
            logger.warning("Found %s inaccessible states and %s states with no starting points.", inaccessible_states,
                           nonstarting_states)
        ...
        while convergence > 1e-6:
            ...
            for k, rhok in enumerate(rho):
                ...
                if rhok == 0 and crossterm > 0:
                    logger.warning("NOOOO for index %s. Crossterm %s rhok %s", k, crossterm, rhok)
        ...
        while inaccessible_states != last_inaccessible_states or nonstarting_states != last_nonstarting_states:
            ...
            for rowidx, row in enumerate(transition_count):
                ...
                if rowsum == 0:
                    ...
                    accessible_states += 1
        if inaccessible_states > 0 or nonstarting_states > 0:
            logger.warning("Found %s accessible states, %s inaccessible states and %s states with no starting points.",
                           accessible_states,
                           inaccessible_states,
                           nonstarting_states)
