...
def reparametrize_path_iter(path,
    ...
    if arclength_weight is None:
        ...
        raise Exception(
            "Expected arclength weights to be of length %s. Got %s" % (len_path - 1, len(arclength_weight)))
    else:
        # Normalize weight
    ...
    for itr in range(0, max_iterations):
        ...
        dist = np.linalg.norm(new_path - previous_path) / np.linalg.norm(previous_path)
        if dist <= convergence:
            break
...
def change_string_length(stringpath, new_length):
    ...
    if new_length < old_length:
        ...
        frac = int(frac)
        new_length = int(frac * new_length - 1)  # minus one since we want the endpoints to be the same
    else:
        frac = None
        reduce_length = False
    ...
    new_path = reparametrize_path_iter(new_path, arclength_weight=new_arclength_weight)
    if reduce_length:
        new_path, new_arclength_weight = new_path[::frac], new_arclength_weight[::frac]
        new_path[-1] = stringpath[-1]  # Just to make sure last point is the same
        if len(new_arclength_weight) >= len(new_path):
            new_arclength_weight = new_arclength_weight[0:orig_new_length - 1]
        new_path = reparametrize_path_iter(new_path, arclength_weight=new_arclength_weight)
    return new_path
